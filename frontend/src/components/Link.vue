<template>
	<div v-if="!field.hidden" class="w-full">
		<div v-if="!matrix">
			...
			<p
				v-if="fieldParsedDescription?.cenrieo"
		...
		<Matrix
			...
			:isFieldMandatory="isFieldMandatory(field)"
			:index="index"
		/>
		<!-- {{field}} -->

		<Matrix1
			v-if="is_matrix_transpose"
			:matrix_code="matrix_code"
			:field="field"
			:modelValue="modelValue"
			@update:modelValue="updateValue"
		...
		<template v-else>
			<div
				:class="table_matrix ? `border-b   pl-2 border-gray-200 bg mx-0` : ``"
				class="flex items-center justify-between"
			>
				<label
					:class="table_matrix ? ` my-2   ` : ``"
					v-if="!is_matrix_transpose && !matrix"
			...
			<p
				...
				class="text-md font-medium text-gray-900 dark:text-gray-200 mb-2"
			>
			...
			<div v-else :class="table_matrix ? `p-2` : ``" class="grid gap-2 mt-2 custom">
				<label
					v-if="!is_matrix_transpose && !matrix"
		...
		<DisclaimerPopup
			v-if="disclaimerPopup"
			:fieldValue="newProgramArea"
